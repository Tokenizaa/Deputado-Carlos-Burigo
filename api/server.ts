...
import { supabaseAdmin, getPublicSettings, getPublicPages, getPublicProjects, getPublicResults, getPublicMunicipalities, getPublicVideos, getPublicMedia, getPublicNews, getPublicAgenda, getPublicDemandByProtocol, getAdminUserById, getAllAdminUsers, getAdminAuditLogs } from '../server/supabase.js';
...
app.get('/api/auth/me', async (req, res) => { try { const userId = (req.headers['x-user-id'] as string) || undefined; let user = null; if (userId) { user = await getAdminUserById(userId); } const allUsers = await getAllAdminUsers(); res.json({ user, allUsers }); } catch (error) { console.error('[api/auth/me] error:', error); res.status(500).json({ error: 'Failed to fetch user' }); } });
...
app.get('/api/videos', async (_req, res) => { try { const videos = await getPublicVideos(); const mappedVideos = videos.map(mapToPublicVideoDto); res.json(mappedVideos); } catch (err: any) { console.error('[api/videos] Supabase error:', err); res.status(500).json({ error: 'Falha ao carregar vídeos do acervo' }); } });
app.get('/api/media', async (_req, res) => { try { const media = await getPublicMedia(); const mappedMedia = media.map(mapToPublicMediaDto); res.json(mappedMedia); } catch (err: any) { console.error('[api/media] Supabase error:', err); res.status(500).json({ error: 'Falha ao carregar mídia do acervo' }); } });
...
app.post('/api/citizen/demand', async (req, res) => { 
  ...
  try { 
    // Generate protocol using the local db's method
    const protocol = db.generateNextProtocol();
    const now = new Date().toISOString();
    const demandId = `dem-${Date.now()}`;
    const historyId = `hist-${Date.now()}`;

    // Insert into the demands table
    const { data: demandData, error: demandError } = await supabaseAdmin
      ...
      .insert({
        ...
        citizen_email: citizenEmail.trim().toLowerCase(),
    ...
    // Insert into the demand_history table
    ...
    // Now fetch the complete demand with messages and history
    const demand = await getPublicDemandByProtocol(protocol);
    if (!demand) {
      throw new Error('Demand not found after insertion');
    }
...
app.get('/api/citizen/lookup', async (req, res) => { const query = req.query.protocol as string; if (!query) return res.status(400).json({ error: 'Informe o número do protocolo' }); const demand = await getPublicDemandByProtocol(query); if (!demand) return res.status(404).json({ error: 'Protocolo não localizado no sistema do gabinete' }); res.json({ protocol: demand.protocol, citizenName: demand.citizenName, municipality: demand.municipality, subject: demand.subject, description: demand.description, category: demand.category, status: demand.status, priority: demand.priority, messages: demand.messages, history: demand.history, createdAt: demand.createdAt, updatedAt: demand.updatedAt }); });
app.get('/api/demands', (_req, res) => res.json(db.getDemands()));
app.get('/api/users', async (_req, res) => { try { res.json(await getAllAdminUsers()); } catch (err: any) { console.error('[api/users] Supabase error:', err); res.status(500).json({ error: 'Falha ao carregar usuários' }); } });
app.get('/api/audit-logs', async (_req, res) => res.json(await getAdminAuditLogs()));
