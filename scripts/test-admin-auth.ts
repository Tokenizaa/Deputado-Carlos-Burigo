#!/usr/bin/env node
/**
 * Admin Auth Validation Test Suite
 * Tests all admin endpoints with ADMIN, EDITOR, COMUNICACAO roles
 * Validates: 200/201 (authorized), 403 (insufficient role), 401 (no token/invalid)
 */
...
async function testEndpoint(
  ...
  const { status, data } = await makeRequest(method, endpoint, token, body);
...
const ADMIN_ENDPOINTS = [
  ...
  { method: 'POST', endpoint: '/api/admin/pages', public: false, body: { title: 'Test Page', slug: 'test-page', status: 'rascunho' } },
  
  // Demands (admin)
  { method: 'GET', endpoint: '/api/demands', public: false },
  
  // Audit logs (ADMIN/EDITOR only)
...
async function runTests() {
  printHeader('ADMIN AUTH VALIDATION TEST SUITE');
  console.log(`Base URL: ${BASE_URL}`);
  console.log(`Test Users: ${tokens.map(t => `${t.role}(${t.userId.slice(0,8)})`).join(', ')}`);

  // Test 1: Public endpoints (no auth required)
  printHeader('TEST 1: Public Endpoints (no auth)');
  for (const ep of PUBLIC_ENDPOINTS) {
    const token = null; // No token
    // We test with each role's token too to ensure they work
    for (const user of tokens) {
      const result = await testEndpoint(ep.method, ep.endpoint, user.role, user.accessToken, 200);
      results.push(result);
    }
  ...
  // Test 2: Admin endpoints - ADMIN role (should have full access)
  printHeader('TEST 2: Admin Endpoints - ADMIN Role (full access)');
  const adminUser = tokens.find(t => t.role === 'ADMIN');
  for (const ep of ADMIN_ENDPOINTS) {
    const expected = ep.method === 'POST' ? 201 : 200;
    const result = await testEndpoint(ep.method, ep.endpoint, 'ADMIN', adminUser?.accessToken || null, expected, ep.body);
    results.push(result);
  }

  // Test 3: Admin endpoints - EDITOR role (should have access to content)
  printHeader('TEST 3: Admin Endpoints - EDITOR Role (content access)');
  const editorUser = tokens.find(t => t.role === 'EDITOR');
  for (const ep of ADMIN_ENDPOINTS) {
    // EDITOR should have access to pages, demands, but not audit-logs (ADMIN only)
    let expected = ep.method === 'POST' ? 201 : 200;
    if (ep.endpoint === '/api/audit-logs') expected = 403; // ADMIN/EDITOR only per RLS
    const result = await testEndpoint(ep.method, ep.endpoint, 'EDITOR', editorUser?.accessToken || null, expected, ep.body);
    results.push(result);
  }

  // Test 4: Admin endpoints - COMUNICACAO role (should have access to news/media)
  printHeader('TEST 4: Admin Endpoints - COMUNICACAO Role (news/media)');
  const comUser = tokens.find(t => t.role === 'COMUNICACAO');
  for (const ep of ADMIN_ENDPOINTS) {
    // COMUNICACAO should have access to pages (for news pages), but not audit-logs
    let expected = ep.method === 'POST' ? 201 : 200;
    if (ep.endpoint === '/api/audit-logs') expected = 403;
    const result = await testEndpoint(ep.method, ep.endpoint, 'COMUNICACAO', comUser?.accessToken || null, expected, ep.body);
    results.push(result);
  }

  // Test 5: No token / invalid token
  printHeader('TEST 5: Unauthorized Access (401)');
  for (const ep of ADMIN_ENDPOINTS) {
  ...
  const result = await testEndpoint('GET', '/api/admin/pages', 'INVALID_TOKEN', 'invalid-token-xyz', 401);
  results.push(result);

  // Test 6: Page Builder Full Flow (ADMIN)
  printHeader('TEST 6: Page Builder Full Flow (ADMIN)');
...
async function testPageBuilderFlow(adminUser: any) {
  ...
  // 1. Create a page
  console.log('\n📄 Creating page...');
  const createRes = await makeRequest('POST', '/api/admin/pages', token, {
  ...
  // 2. Get the page
  console.log('\n📖 Getting page...');
  const getRes = await makeRequest('GET', `/api/admin/pages/${page.id}`, token);
  ...
  // 3. Update the page (add a block)
  console.log('\n✏️ Updating page...');
  const updateRes = await makeRequest('PUT', `/api/admin/pages/${page.id}`, token, {
  ...
  // 4. Publish the page
  console.log('\n🚀 Publishing page...');
  const publishRes = await makeRequest('PUT', `/api/admin/pages/${page.id}`, token, {
    ...updateRes.data,
    status: 'publicado',
    publish: true,
  });
  ...
  console.log('\n📜 Checking versions...');
  const versionsRes = await makeRequest('GET', `/api/admin/pages/${page.id}`, token);
  ...
  if (versionsRes.data.versions && versionsRes.data.versions.length >= 2) {
    const v1 = versionsRes.data.versions[1]; // First version (v1)
    const rollbackRes = await makeRequest('POST', `/api/admin/pages/${page.id}/rollback`, token, {
      versionId: v1.id,
    });
  ...
  // 7. Verify public access to published page
  ...
  // 8. Cleanup - delete test page (if DELETE implemented)
  console.log('\n🧹 Cleanup...');
  // Note: DELETE not implemented in current worker, skipping
}
