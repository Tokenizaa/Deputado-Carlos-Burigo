          ...
          <>
            <div className="mb-14 sm:mb-16 bg-[#141414] border border-stone-800 rounded-[2px] overflow-hidden shadow-2xl"><div className="grid grid-cols-1 lg:grid-cols-12 items-center">
              <div onClick={() => setSelectedVideo(mainVideo)} className="lg:col-span-7 aspect-video bg-black relative cursor-pointer group overflow-hidden">
                <img src={mainVideo.thumbnail || '/assets/alrs_parlamento.jpg'} alt={mainVideo.title} className="w-full h-full object-contain filter contrast-[1.05] transition-transform duration-300 opacity-85 group-hover:opacity-100" loading="lazy" width={960} height={540} referrerPolicy="no-referrer" onError={(e) => { (e.target as HTMLImageElement).src = '/assets/alrs_parlamento.jpg'; }} />
              ...
              <div className="lg:col-span-5 p-6 sm:p-10 flex flex-col justify-between space-y-6"><div className="space-y-4"><div className="flex items-center gap-2 text-xs font-semibold text-[#00A550] uppercase tracking-wider"><span>{videoTypeLabel(mainVideo)}</span><span className="text-stone-700">•</span><span>{sourceLabel(mainVideo)}</span><span className="text-stone-700">•</span><span className="text-stone-400">{new Date(mainVideo.date).toLocaleDateString('pt-BR', { year: 'numeric', month: 'long' })}</span></div><h3 className="text-2xl sm:text-3xl font-bold text-white leading-tight">{mainVideo.title}</h3><p className="text-stone-300 text-sm sm:text-base leading-relaxed">{mainVideo.description}</p></div><div className="pt-4 border-t border-stone-800 flex items-center justify-between"><span className="text-xs text-stone-400">Registro audiovisual do acervo</span><button type="button" onClick={() => setSelectedVideo(mainVideo)} className="h-[44px] px-6 bg-[#00A550] hover:bg-emerald-600 text-white font-bold text-xs uppercase tracking-wider rounded-[2px] transition-all inline-flex items-center gap-2 cursor-pointer"><Play className="w-3.5 h-3.5 fill-white" /><span>ASSISTIR</span></button></div></div>
            </div></div>
            {otherVideos.length > 0 && <div className="space-y-6"><div className="flex items-center justify-between border-b border-stone-800 pb-3"><h4 className="text-lg sm:text-xl font-bold text-white uppercase tracking-wider">OUTROS REGISTROS</h4><span className="text-xs text-stone-400">{otherVideos.length} vídeos</span></div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{otherVideos.map((vid) => <div key={vid.id} onClick={() => setSelectedVideo(vid)} className="bg-[#141414] border border-stone-800 rounded-[2px] overflow-hidden group cursor-pointer hover:border-stone-600 transition-colors flex flex-col justify-between"><div><div className="relative aspect-video bg-black overflow-hidden"><img src={vid.thumbnail || '/assets/alrs_parlamento.jpg'} alt={vid.title} className="w-full h-full object-contain transition-transform duration-300 opacity-80 group-hover:opacity-100" loading="lazy" width={640} height={360} referrerPolicy="no-referrer" onError={(e) => { (e.target as HTMLImageElement).src = '/assets/alrs_parlamento.jpg'; }} /><div className="absolute inset-0 bg-black/30 flex items-center justify-center group-hover:bg-black/15 transition-colors"><div className="w-12 h-12 rounded-full bg-[#00A550] text-white flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform"><Play className="w-5 h-5 fill-white ml-0.5" /></div></div><div className="absolute bottom-2 left-2 glass-card-dark px-2 py-0.5 rounded-[2px] text-[10px] font-bold text-white uppercase tracking-wider">{isShort(vid) ? 'SHORT' : (vid.category || sourceLabel(vid))}</div></div><div className="p-5 space-y-2"><div className="flex items-center gap-2 text-xs text-stone-400"><span className="font-bold text-[#00A550]">{sourceLabel(vid)}</span><span>•</span><span>{new Date(vid.date).toLocaleDateString('pt-BR')}</span></div><h5 className="font-bold text-white text-base leading-snug group-hover:text-[#00A550] transition-colors">{vid.title}</h5><p className="text-xs text-stone-400 line-clamp-2 leading-relaxed">{vid.description}</p></div></div><div className="p-5 pt-0 text-xs font-semibold text-emerald-400 flex items-center gap-1"><span>Assistir vídeo</span><span>→</span></div></div>)}</div></div>}
          </>
        )}
        {selectedVideo && <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-3 sm:p-6" role="dialog" aria-modal="true" aria-label={selectedVideo.title} onClick={() => setSelectedVideo(null)}>
          <div className="bg-[#141414] text-white rounded-xl border border-stone-700 w-full max-w-5xl max-h-[95vh] overflow-hidden shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-center justify-between gap-4 p-4 sm:p-5 border-b border-stone-800">
              <div className="min-w-0"><div className="flex items-center gap-2 mb-1"><span className="text-[10px] font-black uppercase tracking-wider text-[#00A550]">{videoTypeLabel(selectedVideo)}</span><span className="text-stone-700">•</span><span className="text-[10px] text-stone-400">{sourceLabel(selectedVideo)}</span></div><h4 className="font-bold text-sm sm:text-lg text-white truncate">{selectedVideo.title}</h4></div>
              <button type="button" onClick={() => setSelectedVideo(null)} className="shrink-0 w-10 h-10 flex items-center justify-center text-stone-400 hover:text-white hover:bg-stone-800 rounded-lg cursor-pointer" aria-label="Fechar vídeo"><X className="w-6 h-6" /></button>
            </div>
            <div className={isShort(selectedVideo) ? "bg-black flex justify-center" : "aspect-video bg-black"}>
              {getYouTubeId(selectedVideo.url) ? <div className={isShort(selectedVideo) ? "aspect-[9/16] h-[min(78vh,720px)] max-w-[405px]" : "w-full h-full"}><iframe src={`https://www.youtube-nocookie.com/embed/${getYouTubeId(selectedVideo.url)}?autoplay=1&rel=0`} title={selectedVideo.title} className="w-full h-full border-0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /></div> : <div className="min-h-72 flex items-center justify-center p-8 text-center"><div className="space-y-4"><p className="text-stone-300 text-sm">Este conteúdo está hospedado na plataforma registrada no acervo.</p><a href={selectedVideo.url} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center justify-center bg-[#00A550] text-white px-6 rounded-lg text-sm font-bold uppercase tracking-wider hover:bg-emerald-600 transition-colors">Abrir fonte original</a></div></div>}
            </div>
            <div className="p-4 sm:p-5 border-t border-stone-800"><p className="text-xs text-stone-400 line-clamp-2">{selectedVideo.description || 'Registro audiovisual do acervo público.'}</p></div>
          </div>
        </div>}
      </div>
    </section>
