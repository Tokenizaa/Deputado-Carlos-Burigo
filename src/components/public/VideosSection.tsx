      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {selectedVideo && <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-3 sm:p-6" role="dialog" aria-modal="true" aria-label={selectedVideo.title} onClick={() => setSelectedVideo(null)}>
          <div className="bg-[#141414] text-white rounded-xl border border-stone-700 w-full max-w-5xl max-h-[95vh] overflow-hidden shadow-2xl" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-t border-stone-800"><p className="text-xs text-stone-400 line-clamp-2">{selectedVideo.description || 'Registro audiovisual do acervo público.'}</p><a href={selectedVideo.url} target="_blank" rel="noreferrer" className="shrink-0 text-xs font-bold text-[#00A550] hover:text-emerald-400">Abrir no original ↗</a></div>
          </div>
        </div>}
      </div>
    </section>
